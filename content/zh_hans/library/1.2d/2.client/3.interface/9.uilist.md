...
```ts
// 当改变状态时派发 onChange(state:number) state=0 表示selectedIndex改变，否则是overIndex
EventObject.CHANGE 
// 加载完成时候事件
EventObject.LOADED 
// 打开状态发生改变时
UIList.OPEN_STATE_CHANGE 
// 点击确认项
UIList.ITEM_CLICK 
// 【EventUtils事件】当焦点更改时派发的事件
UIList.EVENT_FOCUS_CHANGE 
// 创建项时
UIList.ITEM_CREATE 
```
...
| **[scrollTo](#scrollto)**(index : number,  ignoreAlreadyInVisible? : boolean,  tween? : boolean,  duration? : number,  ease? : Function,  complete? : Callback): void<br>滚动到指定行：如果该项显示对象在完全显示中时则忽略滚动 |
...
【静态】事件：当焦点更改时派发的事件 onFocusChange(lastFocus:UIList,currentFocus:UIList);
...
【静态】事件：打开状态发生改变时 onChange(ui,data)
...
【静态】事件：点击确认项（已选中该项时再点击则派发事件/或ENTER键）
...
【静态】事件：创建项时 onItemCreate(ui: UIRoot, data: UIListItemData,index:number)
...
【静态】开启单一焦点系统（焦点在指定的List上才可以操作，否则默认是无法操作的状态）
