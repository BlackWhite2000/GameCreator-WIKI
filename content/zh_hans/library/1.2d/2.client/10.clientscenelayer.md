...
|                                                                                                                                                                                                                                                                            |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[constructor](#constructor)**(scene : ClientScene)<br>构造函数                                                                                                                                                                                                           |
| **[refreshLoopShow()](#refreshloopshow)**: void<br>刷新循环显示，中途更改了xLoop或yLoop后调用此项以便刷新                                                                                                                                                                  |
| **[drawTile](#drawtile)**(xGrid: number, yGrid: number, tileData: { tex: Texture; texID: number; x: number; y: number; w: number; h: number; }): void<br>绘制图块，绘制后需要调用flushTile进行冲印，同时绘制多个图块时可以在绘制完毕后统一冲印，以便节约不必要的性能耗损。 |
...
```ts
drawTile(xGrid: number, yGrid: number, tileData: { tex: Texture; texID: number; x: number; y: number; w: number; h: number; })
```
