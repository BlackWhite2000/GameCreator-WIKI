...
| [constructor](#constructor)(so : SceneObjectEntity, loop : boolean, targetSceneObject : SceneObject, onOver : Callback, startIndex? : number, executor? : SceneObjectEntity)<br>构造函数 |
...
| [reset](#reset)(defSceneObejct : SceneObject): void<br>[编辑器预览用]重置：还原到最初始的状态                                                                                            |
...
【静态】当前行为层索引变更时派发 EventUtils.happen(this, SceneObjectBehaviors.EVENT_INDEX_CHANGE, [v]);
...
```ts [签名]
toBehaviorData(behaviorStr : string) :
```
...
- #### 返回
  ...
  - `targetSceneObjectIndex` 0-对象 -2玩家 -1当前对象 0-N 指定对象编号
  - `behaviorData` 行为数据
  - `loop` 是否循环
  - `cover` 是否覆盖行为
