...
class KeyboardControl {
     ...
     * -- 第二判断：优先按照方向前一格
     */
    static tryTriggerSceneObjectClickEvent() {
        let d = Game.player.sceneObject.avatarOri;
    ...
    static update() {
        ...
        let xGrid = Math.floor(Game.player.sceneObject.x / Config.SCENE_GRID_SIZE) + this.dirOffsetArr[this.dir][0];
        let yGrid = Math.floor(Game.player.sceneObject.y / Config.SCENE_GRID_SIZE) + this.dirOffsetArr[this.dir][1];
        if (ClientWorld.data.moveToGridCenter) {
            toX = xGrid;
            toY = yGrid;
        }
     ...
     * @return [boolean]
     */
    static dirKeyDownTrue(keyCode, isDown) {
     ...
     * @param yGrid 格子坐标点y
     */
    static moveDirectGrid(xGrid, yGrid) {
     ...
     */
    static moveDirect(x, y, trySingleDir = true) {
        Game.player.sceneObject.ignoreCantMove = true;
        Game.player.sceneObject.keepMoveActWhenCollsionObstacleAndIgnoreCantMove = true;
        if (trySingleDir) {
     ...
     * 取消移动结束后触发点击事件（空白事件）
     */
    static offMoveOverTriggerClickEvent() {
