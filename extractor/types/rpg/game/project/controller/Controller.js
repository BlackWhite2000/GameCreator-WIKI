...
class Controller {
     ...
     */
    static startJoy(dirAngle, recordAngle = true, tryTimes = -1, oriAngle = null) {
        ...
        if (WorldData.moveToGridCenter) {
            if (!Game.player.sceneObject.isMoving) {
                ...
                let xGrid = Math.floor(Game.player.sceneObject.x / Config.SCENE_GRID_SIZE) + offset[0];
                let yGrid = Math.floor(Game.player.sceneObject.y / Config.SCENE_GRID_SIZE) + offset[1];
     ...
     */
    static startSceneObjectClickEvent(target, playerFaceToTarget = false) {
        // 如果处于暂停阶段的话忽略
        if (Game.pause)
            return;
        if (target) {
            ...
            if (target.hasCommand[0]) {
                ...
                GameCommand.startCommonCommand(14012, [], Callback.New(() => {
                    // 执行点击事件
                    GameCommand.startSceneObjectCommand(target.index, 0, null, Callback.New((target) => {
     ...
     */
    static moveToNearTargetSceneObjectAndTriggerClickEvent(targetSceneObject) {
        // 无事件则忽略
     ...
     */
    static clearNearTargetSceneObjectAndTriggerClickEvent(fromAutoRetry = false) {
        if (fromAutoRetry)
            return;
     ...
     * @return [boolean] 是否成功
     */
    static startSceneObjectTouchEvent(trigger, executor, onCommandExecuteOver = null) {
     ...
     */
    static startSceneObjectTouchOutEvent(trigger, executor, onCommandExecuteOver = null) {
        // 如果处于暂停阶段的话监听游戏暂停状态改变后再重新开始
        if (Game.pause) {
            EventUtils.addEventListener(Game, Game.EVENT_PAUSE_CHANGE, Callback.New(Controller.startSceneObjectTouchEvent, this, [trigger, executor, onCommandExecuteOver]), true);
            return false;
     ...
     * @param isStart 是否开始
     */
    static onCommandStart(enabledID, isStart) {
     ...
     */
    static onJoyCollision(dirAngle, tryTimes, oriAngle) {
        // 尝试改变角度
        if (oriAngle == null)
            oriAngle = dirAngle;
        if (tryTimes == -1) {
     ...
     * @return [number]
     */
    static getCollisionAutoTryAngle(fromAngle, tryTimes) {
        // 判断是否接近四方向的角度
        let nearAngle = 22;
