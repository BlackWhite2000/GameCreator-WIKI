...
class GUI_Shop extends GUI_11 {
    ...
    initGoodsList() {
        ...
        for (let i = 0; i < dataArr.length; i++) {
            ...
            let item = GameData.getModuleData(moduleID, gooldsID);
            // 道具不存在的情况：忽略
            if (!item)
                continue;
            data.data = goods;
    ...
    onGoodsClick() {
        // 获取当前选中的列表项
        let selectedItem = this.goodsList.selectedItem;
        if (selectedItem && selectedItem.data) {
            // 获取商品信息
            let itemID = selectedItem.data; // 道具编号
            let itemNum = Number(selectedItem.itemNum); // 剩余数量
            let itemPrice = Number(selectedItem.itemPrice); // 当前价格
            // 可购买的情况
    ...
    onSellItemClick() {
        // 获取当前选中的列表项
        let selectedItem = this.sellItemList.selectedItem;
        if (selectedItem && selectedItem.data) {
            // 获取待售品信息
            let itemID = selectedItem.data; // 道具编号
            let itemNum = Number(selectedItem.itemNum); // 持有数量
            let itemPrice = Number(selectedItem.itemPrice); // 出售价格
            // 可出售的情况
            if (itemNum > 0) {
                // 播放 确认音效
                GameAudio.playSE(ClientWorld.data.sureSE);
    ...
    onKeyDown(e) {
        ...
        if (this.focusState == 1) {
            ...
            // 方向键：数量操作
            let buyNum = Number(this.buyNum.text);
            let flag_key_num = false;
            // 下：减 10
     ...
     * 购买商品
     */
    buyGoods(selectedItem, buyNum) {
        // 容错
        if (!selectedItem || !selectedItem.data)
            return;
        let itemDS = selectedItem.data;
     ...
     * 减少库存
     */
    reduceGoodsNum(buyNum) {
        // 获取选中项索引
        let selectedItemIndex = this.goodsList.selectedIndex;
        if (selectedItemIndex < 0)
            return;
        // 获取商品数据
        let goods = this.goodsList.items[selectedItemIndex];
        // 获取商店事件原始数据
        let data = this.shopEventData.goodsList[selectedItemIndex];
        // 当数量类型为无限时，什么都不发生
    ...
    refreshGoodsListView() {
        for (let i = 0; i < this.goodsList.length; i++) {
            // 获取对应商品的界面
            let goodsUI = this.goodsList.getItemUI(i);
            // 获取对应商品的数据
            let goods = this.goodsList.items[i];
     ...
     * 刷新待售列表显示效果
     */
    refreshSellItemListView() {
        for (let i = 0; i < this.sellItemList.length; i++) {
            // 获取对应商品的界面
            let sellItemUI = this.sellItemList.getItemUI(i);
            // 获取对应商品的数据
            let sellItem = this.sellItemList.items[i];
            // 根据价格判断是否不可购买更改显示效果（这里默认就是半透明啦~）
     ...
     * 刷新道具详情
     */
    refreshItemInfo(list = this.goodsList) {
        // 获取选中的项数据
        ...
        // 已选中道具的情况：显示该道具详情
            ...
            if (this.typeTab.selectedIndex == 0) {
                ...
                let inPackageItemDS = selectedItem.data;
     ...
     * 刷新持有数量
     */
    refreshItemInPackage(itemID, selectedItem) {
     ...
     * 刷新购买数量
     */
    refreshBuyNum(selectedItem, buyNum) {
        if (!buyNum)
     ...
     * 刷新出售数量
     */
    refreshSellNum(selectedItem, buyNum) {
        if (!buyNum)
            buyNum = Number(this.buyNum.text);
        if (selectedItem && selectedItem.data) {
            // 修正最大出售数量，不超过持有数量
    ...
    refreshSellItemList() {
        ...
        for (let i = 0; i < Game.player.data.package.length; i++) {
            ...
            if (sell == 0)
                continue;
            let sellPrice = Math.ceil(sell * this.shopEventData.discount);
            d.data = itemDS; // 模块编号
            // 绑定数据
