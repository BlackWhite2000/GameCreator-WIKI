...
function uiComponentInit(isRoot, uiComp) {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        if (hasCommand) {
            ...
            uiComp.on(evType, uiComp, (uiComp, i) => {
    ...
    if (uiComp instanceof UIButton || uiComp[hasCommandName][1]) {
        uiComp.on(EventObject.MOUSE_OVER, uiComp, (uiComp) => {
...
// 界面添加时处理
EventUtils.addEventListener(GameUI, GameUI.EVENT_CREATE_UI, Callback.New((ui) => {
...
// -- 当关闭对话框时
//------------------------------------------------------------------------------------------------------
let ___lastListFocus;
let ___lastButtonsFocus;
let ___lastFocusIsList = false;
let ___isRecordFoucs = false;
let playDialogSEEnabled = false;
// 监听对话框开启事件：记录焦点
EventUtils.addEventListener(GameDialog, GameDialog.EVENT_DIALOG_START, Callback.New((isOption, content, options, name, head, expression, audioURL, speed) => {
    // 记录上一个焦点
    if (!___isRecordFoucs) {
        if (UIList.focus) {
...
// 监听对话框开启事件：记录焦点
EventUtils.addEventListener(GameDialog, GameDialog.EVENT_AFTER_DIALOG_START, Callback.New((isOption) => {
    // 选项
    if (isOption) {
        ...
        let f = (state) => {
            if (state == 0) {
                ...
                for (let i = 0; i < optionBtns.length; i++) {
                    let optionSp = optionList.getItemUI(i);
...
// 监听对话框关闭事件：恢复焦点
EventUtils.addEventListener(GameDialog, GameDialog.EVENT_DIALOG_END, Callback.New((gameDialog) => {
    // 关闭的情况：恢复焦点
...
UIList.prototype["itemInit"] = function (ui, data, index) {
    ui.on(EventObject.MOUSE_DOWN, this, (ui, data, index) => {
