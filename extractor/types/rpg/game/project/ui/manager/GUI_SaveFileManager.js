...
class GUI_SaveFileManager {
     ...
     * @param list 档案列表组件
     */
    static initSaveFileList(list, saveMode = false) {
        // 标准化list
     ...
     */
    static saveFile(id, executeEvent = true, onFin = null, waitEventCompleteCallback = true) {
        if (GameGate.STATE_3_IN_SCENE_COMPLETE < 3) {
            onFin && onFin.run();
            return;
        }
        // -- 储存
        SinglePlayerGame.saveGame(id, Callback.New((success) => {
            if (executeEvent) {
                if (onFin && !waitEventCompleteCallback)
                    onFin.run();
                if (success) {
                    let saveUI = GameUI.get(5);
                    if (saveUI)
                        GUI_SaveFileManager.refreshSaveFileItem(saveUI.list);
                    // 储存档案-成功
                    GameCommand.startCommonCommand(14008, [], Callback.New((onFin) => {
                        onFin && onFin.run();
                    }, this, [waitEventCompleteCallback ? onFin : null]));
                }
                else {
                    // 储存档案-失败
                    GameCommand.startCommonCommand(14009, [], Callback.New((onFin) => {
                        onFin && onFin.run();
                    }, this, [waitEventCompleteCallback ? onFin : null]));
                }
            }
    ...
    static loadFile(id, onFin = null) {
        ...
        // 读取存档，失败的话调用失败时事件处理
        SinglePlayerGame.loadGame(id, Callback.New((success, customData) => {
            // 成功时回调
            if (success) {
     ...
     * @param list 列表
     */
    static onSaveFileListDisplay(list) {
        // 设置焦点到List
        UIList.focus = list;
        // 刷新档案列表
     ...
     * @param saveMode
     */
    static onListItemClick(list, saveMode) {
        // 未选中任何项的话忽略
        ...
        if (saveMode) {
            ...
            // 不存在档案数据的话则忽略
            let saveFileData = list.selectedItem.data;
            if (!saveFileData)
                return;
            // 读取存档，失败的话调用失败时事件处理
     ...
     * @param index 档案项索引
     */
    static onCreateSaveFileItem(saveMode, ui, data, index) {
        let saveFileData = data.data;
        // 如果没有档案数据则删除按钮和缩略图都隐藏
        if (ui.screenshotImg)
            ui.screenshotImg.visible = (saveFileData ? true : false);
        if (ui.delBtn) {
            ui.delBtn.on(EventObject.MOUSE_DOWN, this, (e) => { e.stopPropagation(); });
            if (saveFileData) {
     ...
     * @param list 档案列表组件
     */
    static refreshSaveFileItem(list) {
        if (!list)
            return;
        let saveInfo = SinglePlayerGame.getSaveInfo();
        let items = [];
        for (let i = 1; i <= WorldData.saveFileMax; i++) {
            let saveFile = ArrayUtils.matchAttributes(saveInfo, { id: i }, true)[0];
