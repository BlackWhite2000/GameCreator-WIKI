...
class GUI_VirtualKeyboard extends GUI_12 {
    constructor() {
         ...
         * 摇杆中心点
         */
        this.rockerCenterPoint = new Point;
        /**
         * 记录上次的方向键值
         */
        this.lastMenuDir = 0;
    ...
    init() {
        ...
        // 保持该界面最前方显示
        EventUtils.addEventListenerFunction(GameUI, GameUI.EVENT_OPEN_SYSTEM_UI, (uiID) => {
            if (uiID != 12) {
                if (this.stage)
                    GameUI.show(12);
            }
        }, this);
        EventUtils.addEventListenerFunction(GameDialog, GameDialog.EVENT_DIALOG_START, (isOption, content, options, name, head, expression, audioURL, speed) => {
            if (this.stage)
                GameUI.show(12);
        }, this);
    }
    ...
    onVirtualKeyboardMenuDirChange(dir) {
        // 在菜单中支持控制（List）
