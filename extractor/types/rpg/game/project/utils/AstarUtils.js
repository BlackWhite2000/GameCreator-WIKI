...
class AstarUtils {
     ...
     */
    static moveTo(x_x1, x_y1, x_x2, x_y2, gridW, gridH, scene, ori4 = false, toGridAsThroughEnabled = false) {
        let sceneUtils = scene.sceneUtils;
        ...
        let rangeW = Math.floor(Config.WINDOW_WIDTH / GRID_SIZE) + 1;
        let rangeH = Math.floor(Config.WINDOW_HEIGHT / GRID_SIZE) + 1;
        //范围X1，Y1，X2，Y2 (根据这个来计算范围内的值..就是在角色周围)
        let n_f_x1 = Math.floor(x_x1 / GRID_SIZE) - rangeW;
        let n_f_y1 = Math.floor(x_y1 / GRID_SIZE) - rangeH;
        let n_f_x2 = Math.floor(x_x1 / GRID_SIZE) + rangeW;
        let n_f_y2 = Math.floor(x_y1 / GRID_SIZE) + rangeH;
        //超出范围的话就return
        ...
        let offsetX = n_f_x1 - 0;
        let offsetY = n_f_y1 - 0;
        let yLen = (n_f_y2 - n_f_y1);
        let xLen = (n_f_x2 - n_f_x1);
        ...
        // 查询路线
        let _ARoad = new AstarUtils(); // *是否存在内存溢出？
        _ARoad.ori4 = ori4;
        ...
        for (let i = roadList.length - 1; i > 0; i--) {
            let n_px_real = roadList[i].px + offsetX;
            let n_py_real = roadList[i].py + offsetY;
     ...
     * @param obsArr
     */
    static def_bigMoveTo(gridW, gridH, obsArr) {
        let n_f_x1 = 0;
        let n_f_y1 = 0;
        let n_f_x2 = gridW;
        let n_f_y2 = gridH;
     ...
     * @return [string]
     */
    static bigMoveTo(x_x1, x_y1, x_x2, x_y2) {
        let GRID_SIZE = Config.SCENE_GRID_SIZE;
        let GRID_SIZE_HALF = Math.floor(Config.SCENE_GRID_SIZE / 2) - 1;
        ...
        let roadLines = [];
        for (let i = roadList.length - 1; i > 0; i--) {
            let n_px_real = roadList[i].px;
            let n_py_real = roadList[i].py;
            roadLines.push([n_px_real * GRID_SIZE + GRID_SIZE_HALF, n_py_real * GRID_SIZE + GRID_SIZE_HALF]);
        }
        roadLines.push([x_x2, x_y2]);
        return roadLines;
    ...
    searchRoad(start, end, map) {
        ...
        while (true) {
            ...
            let thisPoint = this.openList.splice(this.getMinF(), 1)[0]; //每次取出开启列表中的第一个节点
    ...
    addAroundPoint(thisPoint) {
        let thisPx = thisPoint.px; //当前节点横向索引
        let thisPy = thisPoint.py; //当前节点纵向索引
    ...
    getMinF() {
        let tmpF = 100000000; //用以存放最小F值（这里先假定了一个很大的数值）
        let id = 0;
        let rid;
