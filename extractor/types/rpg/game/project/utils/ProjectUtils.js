...
class ProjectUtils {
    ...
    static init() {
        // 鼠标滚动值
        stage.on(EventObject.MOUSE_WHEEL, this, (e) => { ProjectUtils.mouseWhileValue = e.delta; });
        // 鼠标移动
        stage.on(EventObject.MOUSE_MOVE, this, (e) => { ProjectUtils.lastControl = 0; });
        // 注册键盘点击事件
        stage.on(EventObject.KEY_DOWN, this, (e) => { ProjectUtils.lastControl = ProjectUtils.fromGamePad ? 2 : 1; ProjectUtils.fromGamePad = false; this.keyboardEvent = e; if (ArrayUtils.matchAttributes(this.keyboardEvents, { keyCode: e.keyCode }, true).length == 0) {
            this.keyboardEvents.push({ keyCode: e.keyCode });
        } });
        // 注册键盘弹起事件
        stage.on(EventObject.KEY_UP, this, (e) => { ArrayUtils.remove(this.keyboardEvents, ArrayUtils.matchAttributes(this.keyboardEvents, { keyCode: e.keyCode }, true)[0]); this.keyboardEvent = null; });
    }
    //------------------------------------------------------------------------------------------------------
    // Rectangle
    //------------------------------------------------------------------------------------------------------
     ...
     */
    static groupElementsMoveIndex(groupElements, currentIndex, moveDir, limitSecondAxis = 50) {
        ...
        if (!allowOris)
            return null;
        // 获取第二轴参考
        let secondAxisName = (moveDir == 4 || moveDir == 6) ? "y" : "x";
        // 遍历所有节点
