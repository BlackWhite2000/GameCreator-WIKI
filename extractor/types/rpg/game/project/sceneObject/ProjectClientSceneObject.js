...
class ProjectClientSceneObject extends SceneObjectCommon {
     ...
     */
    getSaveData() {
        let o = { myLastTouchObjects: [], moveState: null, recordMoveRoadInfo: this.recordMoveRoadInfo, eventStartWaitInfo: this.eventStartWaitInfo };
     ...
     * @return 对象行为处理器
     */
    addBehavior(behaviorData, loop, targetSceneObject, onOver, cover, startIndex = 0, Immediate = true, forceStopLastBehavior = false, delayFrame = 0, executor = null) {
        // 设置对象行为前的处理
        GameCommand.startCommonCommand(14016, [], null, this, this);
        // 新建一层行为，当该层行为结束时，通知并直接立刻继续执行上一层的行为
        let soBehavior = new ProjectSceneObjectBehaviors(this, loop, targetSceneObject, Callback.New((onOver, soBehavior) => {
     ...
     */
    setTo(x, y, stopMove = true, integer = true, alreadySetTempPosHelper = false, alreadySetRect = false, clacTouchEvent = true) {
        if (this.isJumping)
            return;
        // 需要停止移动的场合
        if (stopMove)
     ...
     */
    autoFindRoadMove(toX, toY, ifObstacleHandleMode = 0, costTime = 0, useAstar = true, whenCantMoveRetry = false, useGridObstacle = true, forceDir4 = false, fromAutoRetry = false) {
        // 跳跃中不允许移动
        if (this.isJumping)
            return;
        // 获取目的地
     ...
     */
    startMove(movePath, costTime = 0, useGridObstacle = false, onMoveOver = null) {
        if (this.isJumping)
            return;
     ...
     */
    restoryMove(recordMoveStateInfo, force = false) {
     ...
     */
    refreshCoordinate(alreadySetTempPosHelper = false, alreadySetRect = false, clacTouchEvent = true, triggerEvent = true) {
        if (!this.inScene)
            return;
     ...
     * @param 是否等待成功
     */
    eventStartWait(trigger, faceToTrigger = true) {
     ...
     */
    clearMyTouchRecord(targetSo = null) {
        if (targetSo) {
     ...
     * 判断指定对象是否在我的接触者列表中
     */
    isInMyTouchList(targetSo) {
        return this.myLastTouchObjects.indexOf(targetSo) != -1;
    }
    /**
     * 开始移动（此处不会预先计算障碍，纯粹根据指定的路径移动，但在移动过程中会判定障碍）
     */
    doStartMove(movePath, costTime = 0, useGridObstacle = false, fromAutoRetry = false) {
        // 转为Point数组
        let roadsArr = [new Point(this.x, this.y)];
     ...
     */
    updateCoordinate(_nowTime) {
        let per;
        let isChangeDir = false;
        let isMoveOver = false;
        // 获取当前路段需要花费的时间段（nowRoadTime，新路段需要计算）
        let thisRoadP = this.roadsArr[this.nowRoad];
        let nextRoadP = this.roadsArr[this.nowRoad + 1];
        // 相同坐标的路段剔除掉
        ...
        if (this.nowRoadTime == 0) {
            let thisRoadS = this.thisRoadS = thisRoadP.distance(nextRoadP.x, nextRoadP.y);
            this.nowRoadTime = thisRoadS * 1000 / this.moveSpeed;
            isChangeDir = true;
        }
        // 大于当前路段终点的时间时
        let nowRoadEndDate = this.nowRoadStartDate + this.nowRoadTime;
        ...
        // 计算当前路段
        let nowP = Point.interpolate(nextRoadP, thisRoadP, per);
     ...
     * @return 是否触发过事件
     */
    touchEventHandle(touchRes, triggerEvent = true) {
        // 恢复存档时当前不会触发事件
        ...
        let subtractList = ArrayUtils.compare(touchRes.touchSceneObjects, this.myLastTouchObjects).subtract;
     ...
     * 重试自动寻路
     */
    retryAutoFindRoadMove(toX, toY, ifObstacleHandleMode = 0, costTime = 0, useAstar = true, whenCantMoveRetry = false, useGridObstacle = true, touchRes = null) {
        if (touchRes && !touchRes.isObstacle)
            return;
        this.clearRetryAutoFindRoadMove(false);
        this.needRetryAutoFindRoadMoveSign = setFrameout(() => {
            if (this.isDisposed || !this.inScene)
                return;
     ...
     * 清理重试自动寻路的状态
     */
    clearRetryAutoFindRoadMove(offCollisionEvent = true) {
     ...
     * 当游戏暂停状态改变时处理
     */
    onGamePauseChangeHandle() {
        // 暂停/恢复全部场景相关的触发器
        for (let i in this.triggerLines) {
            let trigger = this.triggerLines[i];
