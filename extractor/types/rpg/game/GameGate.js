...
class GameGate {
    ...
    static onWorldInit() {
        ...
        if (onceLoadGame && onceLoadGame.id != null && SinglePlayerGame.getSaveInfoByID(onceLoadGame.id)) {
            ...
            GUI_SaveFileManager.loadFile(onceLoadGame.id, Callback.New((success) => {
                if (!success)
     ...
     */
    static onInNewScene(sceneID, inNewSceneState) {
        // 如果正处于切换场景中的话忽略掉
        ...
        function onLoadedNewScene(scene) {
            ...
            if (inNewSceneState != 2) {
                let sceneObjects = scene.getPresetSceneObjectDatas();
            ...
            // 背景音乐播放
            if (inNewSceneState != 2) {
                new SyncTask(GameGate.bgmSyncTaskName, (sceneBgm, bgmVolume, bgmPitch) => {
                    if (sceneBgm) {
                ...
                new SyncTask(GameGate.bgsSyncTaskName, (sceneBgs, bgsVolume, bgsPitch) => {
                    if (sceneBgs) {
     ...
     */
    static addPlayerSceneObject(so, isEntity = false, insertNewPostion = true) {
        if (!Game.currentScene)
            return;
        if (!isEntity)
            delete so.player;
